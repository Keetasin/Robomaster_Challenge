        ...
        for i in circles[0, :]:
            ...
            for contour in contours:
                ...
                if circularity > 0.65 :
                    ...
                    cv2.rectangle(frame, top_left, bottom_right, (0, 0, 255), 2)  # สีฟ้า

                    w = bottom_right[0] - top_left[0]
                    h = bottom_right[1] - top_left[1]

                    x,y,w,h = top_left[0]-15, top_left[1]-5, w+27, h+210
...
def detect():
    ...
    for _ in range(1):
        for _ in range(3):
            time.sleep(0.5)
            frame = ep_camera.read_cv2_image(strategy="newest", timeout=0.5)[350:650,200:1070]
...
def sharp_sen_data():
    ...
    print(f"Left: {adc_l_new} Right: {adc_r_new}")
...
def adjust_wall_l():
    if adc_l_new <= 40:
        if adc_l_new < 10:
            # walk_y = (abs(25 - adc_l_new)/100)-0.02
            print("Move right")
            ep_chassis.move(x=0, y=0.15, z=0, xy_speed=MAX_SPEED).wait_for_completed()
        
        # else:
            # ep_chassis.drive_wheels(w1=0, w2=0, w3=0, w4=0)
        # elif adc_l_new > 35:
        #     walk_y = (abs(35 - adc_l_new)/100)-0.019
        #     # print("Move left")
...
def adjust_wall_r():
    if adc_r_new < 40:
        if adc_r_new <= 10  :
            # walk_y = (abs(25 - adc_r_new)/100)-0.02
            print("Move left")
            ep_chassis.move(x=0, y=-0.15, z=0, xy_speed=MAX_SPEED).wait_for_completed()
...
def adjust_wall_f():    
    if tof_distance < 150:
        ...
        print(">>> move back :", walk_x)
    
    if 450 >= tof_distance > 250:
        walk_x = (abs(tof_distance -300)/1000)+0.02
        ep_chassis.move(x=walk_x, y=0, z=0, xy_speed=MAX_SPEED).wait_for_completed()
        print(">>> move forward :", walk_x)

    # if tof_distance < 150:

    #     ep_chassis.move(x=0, y=0, z=0, xy_speed=MAX_SPEED).wait_for_completed()
...
def logic():
    ...
    if visit_counts[row][col] < 2:
        ...
        ep_gimbal.moveto(pitch=0, yaw=-90, pitch_speed=120, yaw_speed=120).wait_for_completed()
        sl = tof_distance
        ep_gimbal.moveto(pitch=0, yaw= 0, pitch_speed=120, yaw_speed=120).wait_for_completed()
        tof = tof_distance
        ep_gimbal.moveto(pitch=0, yaw= 90, pitch_speed=120, yaw_speed=120).wait_for_completed()
        sr = tof_distance
        ...
        print("cerrent :", grid[cerrent_po[0]][cerrent_po[1]])

        front_wall = grid[cerrent_po[0]][cerrent_po[1]][0]
        left_wall = grid[cerrent_po[0]][cerrent_po[1]][1]
        right_wall = grid[cerrent_po[0]][cerrent_po[1]][2]

        print(f"front_wall : {front_wall}, left_wall : {left_wall}, right_wall : {right_wall}")
        print("\n")
