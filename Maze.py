...
def move_until_tof_less_than(ep_chassis, threshold_distance, max_time, overall_start_time, time_data, list_current_x, target):
    start_time = time.time()
    ...
    while (time.time() - start_time) < max_time:
        ...
        list_current_x.append(current_x)
        target.append(None)  # No specific target
        time_data.append(time.time() - overall_start_time)
...
if __name__ == '__main__':
    ...
    for _ in range(4):
        move_until_tof_less_than(ep_chassis, 500, 10, overall_start_time, time_data, list_current_x, target)  # Adjust max_time as needed
        rotate_180_degrees(ep_chassis)
        ep_gimbal.recenter().wait_for_completed()
        time.sleep(0.5)
